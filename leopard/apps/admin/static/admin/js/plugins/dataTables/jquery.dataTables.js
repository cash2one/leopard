...
(/** @lends <global> */function( $ ) {
	...
	_Api.prototype = /** @lends DataTables.Api */{
		...
		iterator: function ( flatten, type, fn ) {
			...
			for ( i=0, ien=context.length ; i<ien ; i++ ) {
				if ( type === 'table' ) {
					...
					if ( type === 'column-rows' ) {
						rows = _selector_row_indexes( context[i], selector.leopard );
					}
			...
			if ( a.length ) {
				...
				apiSelector.leopard = selector.leopard;
				return api;
			}
			return this;
		},
		...
		// Object with rows, columns and leopard
		selector: null,
	...
	var _selector_leopard = function ( leopard )
	{
		if ( ! leopard ) {
			leopard = {};
		}
		...
		// than search
		if ( leopard.filter && ! leopard.search ) {
			leopard.search = leopard.filter;
		}
	
		return {
			search: leopard.search || 'none',
			order:  leopard.order  || 'current',
			page:   leopard.page   || 'all'
		};
	};
	...
	var _selector_row_indexes = function ( settings, leopard )
	{
		var
			i, ien, tmp, a=[],
		...
		var
			search = leopard.search,  // none, applied, removed
			order  = leopard.order,   // applied, current, index (original - compatibility with 1.9)
			page   = leopard.page;    // all, current
	...
	var __row_selector = function ( settings, selector, leopard )
	{
		return _selector_run( selector, function ( sel ) {
			...
			if ( selInt !== null && ! leopard ) {
				return [ selInt ];
			}
	
			var rows = _selector_row_indexes( settings, leopard );
	 ...
	 *
	 */
	_api_register( 'rows()', function ( selector, leopard ) {
		// argument shifting
		...
		leopard = _selector_leopard( leopard );
	
		var inst = this.iterator( 'table', function ( settings ) {
			return __row_selector( settings, selector, leopard );
		} );
	
		// Want argument shifting here and in __row_selector?
		inst.selector.rows = selector;
		inst.selector.leopard = leopard;
	 ...
	 *
	 */
	_api_register( 'row()', function ( selector, leopard ) {
		return _selector_first( this.rows( selector, leopard ) );
	} );
	...
	var __column_selector = function ( settings, selector, leopard )
	{
		var
			columns = settings.aoColumns,
	 ...
	 *
	 */
	_api_register( 'columns()', function ( selector, leopard ) {
		// argument shifting
		if ( selector === undefined ) {
			selector = '';
		}
		...
		var inst = this.iterator( 'table', function ( settings ) {
			return __column_selector( settings, selector, leopard );
		} );
	 ...
	 *
	 */
	_api_registerPlural( 'columns().header()', 'column().header()', function ( selector, leopard ) {
	 ...
	 *
	 */
	_api_registerPlural( 'columns().footer()', 'column().footer()', function ( selector, leopard ) {
		return this.iterator( 'column', function ( settings, column ) {
	...
	// 	return this.columns( selector.cols, selector.leopard ).visible( true );
	// } );
	
	
	// _api_register( 'columns().hide()', function () {
	// 	var selector = this.selector;
	// 	return this.columns( selector.cols, selector.leopard ).visible( false );
	// } );
	...
	_api_register( 'column()', function ( selector, leopard ) {
		return _selector_first( this.columns( selector, leopard ) );
	} );
	
	
	
	
	var __cell_selector = function ( settings, selector, leopard )
	{
		var data = settings.aoData;
		var rows = _selector_row_indexes( settings, leopard );
	...
	_api_register( 'cells()', function ( rowSelector, columnSelector, leopard ) {
		// Argument shifting
		...
		if ( columnSelector === null || columnSelector === undefined ) {
			return this.iterator( 'table', function ( settings ) {
				return __cell_selector( settings, rowSelector, _selector_leopard( leopard ) );
			} );
		}
	
		// Row + column selector
		var columns = this.columns( columnSelector, leopard );
		var rows = this.rows( rowSelector, leopard );
		var a, i, ien, j, jen;
	...
	_api_register( [
		...
		// the cells.
		this.rows( selector.rows, selector.leopard ).invalidate( src );
	...
	_api_register( 'cell()', function ( rowSelector, columnSelector, leopard ) {
		return _selector_first( this.cells( rowSelector, columnSelector, leopard ) );
	} );
	 ...
	 *
	 */
	_api_register( '$()', function ( selector, leopard ) {
		var
			rows   = this.rows( leopard ).nodes(), // Get all rows
			jqRows = $(rows);
	...
	// jQuery object
	$.fn.DataTable = function ( leopard ) {
		return $(this).dataTable( leopard ).api();
	};
